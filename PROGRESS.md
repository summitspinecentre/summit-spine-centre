...
- [x] `/public/images/conditions/tech-neck/hero.png` — added (1408×768)
...
| 2026-05-15 | Home — "What brings you in?" photo pills + stagger | New components/ui/ConditionPillGrid.tsx: 'use client', framer-motion staggerChildren (0.12s interval, delayChildren 0.1s, viewport margin -100px so fires well into scroll), each pill fades+slides-up 0.55s; useReducedMotion safe; full-bleed next/image background per pill, bottom-up eerie-black gradient overlay, white label + ArrowRight, hover scale-[1.06] + opacity-85 overlay; types/content.ts: added image? to conditionsBar.conditions; content/pages.ts: 6 image paths added (low-back-pain → new hero.jpg copied from Images/Back pain.jpg, tech-neck uses conditions hub hero as fallback); ConditionsGrid.tsx: added image? to ConditionCardItem, pillMode? prop — renders ConditionPillGrid when true, existing card grid otherwise; app/page.tsx: pillMode added to conditionsBar ConditionsGrid; Conditions Hub (14 plain cards) untouched and verified; 0 TS errors, all 6 pills verified in browser. |
| 2026-05-15 | Nav — Conditions dropdown redesign | types/content.ts: added NavLink + NavGroup interfaces, extended NavItem with groups?/viewAllHref?/viewAllLabel? (children kept for Services flat list); content/pages.ts: conditions navConfig restructured from flat children[] to 3 anatomical groups (Head & Neck / Back & Spine / Whole Body) ordered top-to-bottom; "Back Pain" nav label renamed "Arthritis & Disc Degeneration"; Navbar.tsx: onMouseEnter/Leave + item.children checks updated to item.children||item.groups so grouped items open correctly; desktop dropdown branches on item.groups — 3-col grid with uppercase micro-headers + border-b dividers, border-l-2 border-cerulean hover on each item (replacing bg-hemlock-50), "View all conditions →" footer row with top divider; mobile accordion: groups render with non-clickable section headings + same cerulean border-l hover; 0 TS errors, verified in browser (dark hero + light nav). |
| 2026-05-15 | "Sound Familiar?" — 3-col redesign | types/content.ts: added icon? to DemographicCondition, image? to DemographicTab; content/pages.ts: all 6 tabs updated with image paths (/images/home/demographics/[id].jpg — awaiting real photos) and per-condition lucide icons (ArrowDown/Monitor/Brain/Activity/Zap/Layers/Dumbbell/Trophy/Baby/Minimize2/HelpCircle); DemographicTabBar.tsx: full redesign — 3-col grid (210px sidebar + 1fr photo + 340px conditions), left col = vertical pill buttons (horizontal scroll on mobile) with ARIA tablist/tab/tabpanel, centre = stacked next/image panels with opacity crossfade (500ms) + bottom-gradient label badge + letter fallback when image missing, right = scrollable conditions panel with lucide icon circle (28px) per row + label + description + ChevronRight for linked rows, first tab auto-selected; 0 TS errors, DOM snapshot verified (6 tabs, 7 condition rows, CTA link). Demographic photos needed in /public/images/home/demographics/. |
